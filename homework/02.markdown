...
# Higher Order Functions for Exercises and Processing Data - __Due Thursday, September 26th, by 11PM__
...
__yelpfunc.js__ and __report.js__ - Print out a report analyzing yelp data. There are two parts to this file:
...
The original data was sourced from [https://www.yelp.com/dataset](https://www.yelp.com/dataset). A subset of the data has been made available through the course site and your repository. You'll be using this data to extract a few analytics.

You'll be using two files for this:

1. `yelpfunc.js` to create a function called `processYelpData`
2. `report.js` to read in JSON data and use the `processYelpData` above to create a report

### Importing Data

* Create a file called `src/report.js`
* Start by reading in the file `tests/business.json` using `fs.readFile` 
...
* Use the `Array` you created above and print the name, rating and location of the 80th business entry in the file in the format: '{name} is a business located in {city} with a rating of {stars} and {reviews} reviews.'
...
* `data` will be an `Array` of JSON objects (each line should have been parsed as a JSON before pushing into the `data` array)
...
1. Create a function in `src/yelpfunc.js`
2. ...that loads in data from the file as explained above, and
3. ...returns a single string that contains a report with the analytics listed below

### Implementation Requirements

When creating your report in processYelpData, you must use all of the following Array methods at least once each in your program:
...
* Start the report with the total number of records in the `business.json` file 

```
Total records read: 2000
=====
```

* This should be followed by 5 equal signs

### Average Rating

* Include the average rating of all the records in the file
...
* Iterate through all the records and find all the restaurants that serve 'Pizza' and are located in the city of 'Las Vegas', in the state of 'NV'. Include each place's rating too.
...
Instead of reading a local file, **modify your program** so that it requests JSON data from a specified url. This URL will be distributed via piazza (see a pinned note). However, there will be one additional object in the last line of the file:

`{"nextFile": "4df1c5ef1280bee2e2b44167027d2469.json"}`

This last object has a key, `nextFile`, and the value represents the name of another file of business data (in the object above, the name of the next file is: `4df1c5ef1280bee2e2b44167027d2469.json`). The next section will explain how to retrieve this json file from the web.
...
* Print out the result of calling `processYelpData` on the parsed response

When you run your report again, the output should remain unchanged despite the change in data source. **Again, feel free to overwrite your existing report.js file (the diffs are in git anyway) for this section.**


### Get Next JSON Files

Instead of running this report on a single file, you'll download several files. However, you will not know the URLs of the subsequent files in advance. Instead, you'll have to examine the `nextFile` property of the last object in initial JSON data requested.

* Print out the URL that you're retrieving before running your report
* The last object from the JSON file contains a `nextFile` property;
* The `nextFile` property contains the next file to retrieve
* Construct a new URL based on that ID and print out the new URL
* Use the `request` library again to retrieve and process the data from this new URL
* Continue to do this until you encounter a file that does not have an object with a `nextFile` property
